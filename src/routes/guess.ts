    ...
    fastify.post('/pools/:poolId/games/:gameId/guesses', { onRequest: [authenticate] }, async (req, res) => {
        ...
        if (!participant) {
            return res.status(400).send({
                message: 'Você não está authorizado para criar'
            })
        }
        ...
        if (guess) {
            return res.status(400).send({
                message: 'Você já fez um palpite a este jogo.'
            })
        }
        ...
        if (game.date < new Date()) {
            return res.status(400).send({
                message: 'Você não pode enviar palpites para jogos após a data do jogo'
            })
        }
